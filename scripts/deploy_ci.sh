...
docker cp artifacts/raptornamer.wasm $CONTAINER_NAME:/raptornamer.wasm
...
# CONTRACT_RES=$($BINARY tx wasm store "/raptornamer.wasm" --from validator $TXFLAG --output json)
# echo $CONTRACT_RES
CONTRACT_CODE=$($BINARY tx wasm store "/raptornamer.wasm" --from validator $TXFLAG --output json | jq -r '.logs[0].events[-1].attributes[0].value')
...
$BINARY tx wasm instantiate $CONTRACT_CODE "$RAPTORNAMER_INIT" --from "validator" --label "raptornamer NFT nameservice" $TXFLAG --no-admin
RES=$?
...
echo "NEXT_PUBLIC_RAPTORNAMER_CODE_ID=$CONTRACT_CODE"
echo "NEXT_PUBLIC_RAPTORNAMER_ADDRESS=$CONTRACT_ADDRESS"
