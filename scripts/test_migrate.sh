...
# upload new wasm
NEW_CONTRACT_CODE=$($BINARY tx wasm store "/raptornamer.wasm" --from validator $TXFLAG --output json | jq -r '.logs[0].events[-1].attributes[0].value')
...
echo "NEXT_PUBLIC_RAPTORNAMER_CODE_ID=$NEW_CONTRACT_CODE"
echo "NEXT_PUBLIC_RAPTORNAMER_ADDRESS=$NEW_CONTRACT_ADDRESS"
